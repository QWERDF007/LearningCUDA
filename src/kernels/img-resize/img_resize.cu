...
__device__ __forceinline__ _Tp saturate_cast(_Tp2 v)
{
    ...
    // {
    //     // OpenCV: return (short)((unsigned)(v - SHRT_MIN) <= (unsigned)USHRT_MAX ? v : v > 0 ? SHRT_MAX : SHRT_MIN);
    //     return (short)((unsigned)(v + 32768) <= 65535U ? v : v > 0 ? 32767 : -32768);
    // }
    // else if constexpr (std::is_same_v<_Tp2, float>)
    // {
    //     int iv = round(v);
    //     return saturate_cast<short>(iv);
    // }
    // else if constexpr (std::is_same_v<_Tp2, double>)
    // {
    //     int iv = round(v);
    //     return saturate_cast<short>(iv);
    // }
