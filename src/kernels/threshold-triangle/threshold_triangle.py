...
sources = [
    str(file.parent / "threshold_triangle.cu")
]
...
def run_benchmark(
    ...
    t,  expected = cv2.threshold(a_np, 128, maxval, cv2.THRESH_TRIANGLE)
...
for H, W in Sizes:
    ...
    a = torch.randint(0, 256, (H, W), dtype=torch.uint8).cuda().contiguous()  # 先用int16防止溢出
    ...
    run_benchmark(lib.threshold_triangle_u8, a, thresh, 255, 'u8_triangle', out)
    run_benchmark(lib.threshold_triangle_u8_warp, a, thresh, 255, 'u8_triangle_warp', out)
    print("-" * 85)