...
sources = [
    str(file.parent / "equalize_hist.cu")
]
...
def run_benchmark(
    ...
    expected = cv2.equalizeHist(a_np)
...
for H, W in Sizes:
    ...
    run_benchmark(lib.equalize_hist, a, 'equalize_hist', out, iters=1000)

    a = torch.randint(0, 256, (H, W), dtype=torch.uint8).cuda().contiguous()
    out = torch.zeros_like(a).cuda().contiguous()
    
    run_benchmark(lib.equalize_hist_opt, a, 'equalize_hist_opt', out, iters=1000)
    print("-" * 85)