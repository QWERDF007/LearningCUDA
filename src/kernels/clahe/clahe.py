...
def run_benchmark(
    ...
    clahe = cv2.createCLAHE(clipLimit, (8, 8))
    expected = clahe.apply(a_np)
...
for H, W in Sizes:
    clipLimit = random.randint(10, 80)
    print("-" * 85)
    print(" " * 40 + f"H={H}, W={W}, ch=1, clipLimit={clipLimit}")

    a = torch.randint(0, 256, (H, W), dtype=torch.uint8).cuda().contiguous()
    out = torch.zeros_like(a).cuda().contiguous()
    
    run_benchmark(lib.clahe, a, clipLimit, 'clahe', out, iters=1000)
