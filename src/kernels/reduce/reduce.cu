...
template<typename T, typename CT>
__global__ void reduce_sum_shared_atomic_kernel(const T *__restrict__ src, CT *__restrict__ dst, const int N)
{
    extern __shared__ char sdata_raw[];
    ...
    if (tid == 0)
    {
        atomicAdd(dst, saturate_cast<T>(sdata[0]));
    }
}

template<typename T, typename CT>
__global__ void reduce_sum_shared_atomic2_kernel(const T *__restrict__ src, CT *__restrict__ dst, const int N)
{
    extern __shared__ char sdata_raw[];
    ...
    // 即使 N 远大于总线程数，也能正确处理所有数据
    CT local_sum = 0;
    while (idx < N)
    {
        local_sum += static_cast<CT>(src[idx]);
        idx += gridSize;
    }
...
#define TORCH_BINDING_REDUCE_SHARED_ATOMIC(tag, th_type, element_type, cal_type, n_pack)                            \
    torch::Tensor reduce_##tag##_##element_type##_##cal_type(torch::Tensor src)                                     \
    {                                                                                                               \
        CHECK_TORCH_TENSOR_DEVICE(src)                                                                              \
        const int H = src.size(0);                                                                                  \
...
#define TORCH_BINDING_REDUCE_SHARED_ATOMIC2(tag, th_type, element_type, cal_type, n_pack)                           \
    torch::Tensor reduce_##tag##_##element_type##_##cal_type(torch::Tensor src)                                     \
    {                                                                                                               \
        CHECK_TORCH_TENSOR_DEVICE(src)                                                                              \
        const int H = src.size(0);                                                                                  \
        ...
        int       N = H * W;                                                                                        \
                                                                                                                    \
        dim3   block(THREADS);                                                                                      \
        dim3   grid(std::min(divUp(N, THREADS), 2048)); /* 限制最大 Block 数 */                                     \
        size_t smem_size = THREADS * sizeof(cal_type);                                                              \
                                                                                                                    \
        auto          options = torch::TensorOptions().dtype(th_type).device(torch::kCUDA, 0);                      \
        torch::Tensor dst     = torch::zeros({1, 1}, options);                                                      \
...
TORCH_BINDING_REDUCE_SHARED_ATOMIC(sum_shared_atomic, torch::kFloat32, float, float, 1)
TORCH_BINDING_REDUCE_SHARED_ATOMIC(sum_shared_atomic, torch::kFloat64, float, double, 1)

TORCH_BINDING_REDUCE_SHARED_ATOMIC2(sum_shared_atomic2, torch::kFloat32, float, float, 1)
TORCH_BINDING_REDUCE_SHARED_ATOMIC2(sum_shared_atomic2, torch::kFloat64, float, double, 1)
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    TORCH_BINDING_COMMON_EXTENSION(reduce_sum_shared_atomic_float_float)
    TORCH_BINDING_COMMON_EXTENSION(reduce_sum_shared_atomic_float_double)
    TORCH_BINDING_COMMON_EXTENSION(reduce_sum_shared_atomic2_float_float)
    TORCH_BINDING_COMMON_EXTENSION(reduce_sum_shared_atomic2_float_double)
}